...
def posts_list_view(request):
    ...
    context = {"posts": posts, "search_form": search_form, "all_users_list": get_all_usernames()}
...
def create_post_view(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            post = create_post(
                title=title, content=content, user=request.user, image=form.cleaned_data["image"]
            )
